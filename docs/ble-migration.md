...
path. As of 2026-09-23 the gateway manifest/Secret are applied and live, and
that day Start → Stop → Start and a Bluetooth toggle both reconnected correctly
on the real device (owner-verified). As of 2026-09-24 the app is at
`versionName` 1.2 (volume/brightness settings, firmware updates over BLE) and
real conversations run through the k3s gateway, with reply audio playing as it
arrives. **Still remaining**: reboot/deep-sleep reconnect and the hours-long
soak. See
`TODO.md`.
