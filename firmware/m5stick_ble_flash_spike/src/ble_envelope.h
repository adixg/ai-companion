// Phase 3 protocol design: the byte-envelope codec both sides speak, re-
// expressing bridge_server.py's existing WebSocket text/binary framing
// (main.cpp's webSocketEvent(), :717 in the real firmware) as a plain byte
// stream, since a BLE characteristic has no text-vs-binary distinction the
// way a WebSocket frame does.
//
...
// -- see main.cpp's CHUNK_BYTES comment in this same spike):
//
...
// totalLen of them, then dispatches (type, payload, totalLen) once. This is
// the exact scheme the migration plan calls for (Phase 3: "1-byte
// frame-type prefix ... 2-byte length prefix on the first physical packet
// ... with a continuation pseudo-type on subsequent packets").
//
...
//     START/STOP/RESET (button presses, today's webSocket.sendTXT calls at
//     main.cpp:941/963/907) and AUDIO_CHUNK (mic audio, today's sendBIN).
//   RX (phone writes, Stick receives): frames relayed down from
//     bridge_server.py via the phone's WebSocket client -- HEARD/STATUS/
//     REPLY/AUDIO_CHUNK(reply audio)/END, plus AUTH and TIME_SYNC (neither
//     of which bridge_server.py sends today; both are new, BLE-only).
//
