...
// BtnB long-press  -> enter true ESP32 deep sleep (µA-range draw). The Stick
//                     has no PMIC-assisted power button wired up for this
//                     (checked M5Unified's board table directly), so BtnB's
//                     GPIO (12) is configured as an EXT0 wake source by hand
//                     before sleeping. Waking is a full chip reset — setup()
//                     runs again from scratch, including reconnecting Wi-Fi —
//                     so it takes a couple of seconds, not an instant resume.
//                     A short tap of BtnB right after wake doesn't count: the
//                     boot re-checks that BtnB is still held for the same
//                     duration a long-press requires, and goes straight back
//                     to sleep otherwise, so a stray bump doesn't pay the
//                     full Wi-Fi-reconnect cost.
//
...
// BtnB's GPIO on this board (M5Unified.cpp's button table: board_M5StickS3 ->
// BtnA=GPIO11, BtnB=GPIO12), and M5Unified's own default Button_Class hold
// threshold — kept in sync manually since the wake-debounce below runs
// before M5's own long-press detection is available (see setup()).
static const gpio_num_t BTN_B_PIN = GPIO_NUM_12;
...
// ---------------------------------------------------------------- deep sleep
// GPIO pull config (unlike pinMode()) isn't retained across deep sleep, so
// it has to be set on the RTC IO peripheral directly; BtnB idles HIGH and
// pulls LOW when pressed (see M5Unified.cpp's btn_rawstate_bits for this
// board), hence the pull-up and the wake level of 0 below. This call never
// returns — the chip powers down and a wake resets it back into setup().
static void configureWakeAndSleep() {
  rtc_gpio_pullup_en(BTN_B_PIN);
  rtc_gpio_pulldown_dis(BTN_B_PIN);
  esp_sleep_enable_ext0_wakeup(BTN_B_PIN, 0);
  esp_deep_sleep_start();
}

// Draws the standby screen and shuts everything down cleanly before actually
// sleeping, so the server sees a proper WS close instead of a timeout and the
// panel goes through its own SLPIN low-power sequence rather than just
// going dark mid-refresh.
...
void setup() {
  ...
  // Woken by BtnB's EXT0 GPIO wake, not a fresh power-on: require it to
  // still be held for the same duration a long-press normally needs before
  // paying for the rest of boot (display/mic/speaker/Wi-Fi) — a brief tap or
  // bump shouldn't force a full reconnect, it should just go back to sleep.
  if (esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_EXT0) {
    uint32_t heldFrom = millis();
    M5.update();
    while (M5.BtnB.isPressed() && millis() - heldFrom < LONG_PRESS_MS) {
      delay(10);
      M5.update();
    }
    if (millis() - heldFrom < LONG_PRESS_MS) {
      configureWakeAndSleep();
    }
  }
...
void loop() {
  ...
  if (M5.BtnB.wasReleasedAfterHold()) {
    enterDeepSleep();  // never returns — chip powers down here
  }
