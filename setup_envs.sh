#!/usr/bin/env bash
# One-shot setup for the voice-assistant: three conda envs + the VITS Space.
#
#   ./setup_envs.sh
#
# Creates:
...
#   chatterbox-tts - runs chatterbox_cli.py  (Chatterbox Turbo TTS, python 3.11)
#
...
#
# Env overrides:  PY_VER (default 3.10), CHAT_ENV, TTS_ENV, CHATTERBOX_ENV,
# CHATTERBOX_PY_VER (default 3.11 — chatterbox-tts is developed/tested on it,
# separately from PY_VER above).
set -euo pipefail
...
cat <<EOF
  ...
  conda run -n $CHATTERBOX_ENV  python chatterbox_cli.py "Hello there." -o /tmp/hi-turbo.wav
  # (first chatterbox_cli.py run downloads the ~350M-param weights from Hugging Face)

run it:
  conda activate $CHAT_ENV
  python chat_loop.py --host http://100.70.0.38:11434 --model qwen3:8b
  # --tts-backend chatterbox is the default now; pass --tts-backend vits for the old voice
EOF
